...
export default function WomenCategory() {
    ...
    const womenCategory = displayProducts?.filter((product) => product.category === "women's clothing")
    const searchProducts = womenCategory.filter((item) => item.title.toLowerCase().includes(search.toLocaleLowerCase()));
                ...
                }
                {
                    searchProducts && searchProducts.length > 0 ?
                        searchProducts.map((product) => {
                            return (
                                <div className='w-[100%] sm:w-[48%] md:w-[32%] lg:w-[23%] mt-[30px] shadow-md border border-gray-300 pt-[30px] rounded-[10px] transition-transform transform hover:scale-105 hover:shadow-xl'>
